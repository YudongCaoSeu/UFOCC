...
class UFOCC:
    def __init__(self, sequence_length=20, stride=1,
                 num_epochs=1000, batch_size=32, lr=0.5*1e-4, ss_type='FULL',
                 hidden_dims=32, emb_dim=10, rep_hidden=32, pretext_hidden=16,
                 kernel_size=5, dropout=0.0, bias=True,
                 alpha=0.01, neg_batch_ratio=0.5, es=False, train_val_pc=0.25,
                 seed=0, device='cuda',
                 logger=None, model_dir='ufocc_model/',
        ...
        hidden_dims: integer or list of integer, default=16,
            the number of neural units in the hidden layer, → → → → → → → →'out_channels'TCN卷积块的输出
        ...
        alpha: float, default=0.1
            the weight of the classification head of laa
        ...
        model_dir: string, default='ufocc_model/'
            directory to store intermediate model files
        laa: bool, default=True
    ...
    def fit(self, X: pd.DataFrame):
        ...
        sequences = sequences[RandomState(42).permutation(len(sequences))] ##打乱排序

##Few-shot中train_val非必须
    ...
    def train(self, net, train_seqs, val_seqs=None):
        ...
        criterion_oc_scc = DSVDDUncLoss(c=self.c, reduction='mean')
        criterion_mse = torch.nn.MSELoss(reduction='mean')

        early_stp = EarlyStopping(intermediate_dir=self.model_dir,
                                  patience=7, delta=1e-6, model_name='ufocc', verbose=False)
        ...
        for i in range(self.num_epochs):
            ...
            for ii, x0 in enumerate(train_loader):
                ...
                if self.scc:
                    ...
                    loss_oc = criterion_oc_scc(rep_x0, rep_x0_dup)
                else:
                    rep = x0_output[0]
            ...
            if val_seqs is not None:
                ...
                with torch.no_grad():
                    for x in val_loader:
                        ...
                        x_out = net(x)
                        if self.scc:
                            loss = criterion_oc_scc(x_out[0], x_out[1])
                        else:
    ...
    def predict(self, X):
        ...
        dis_pad = np.hstack([0 * np.ones(data.shape[0] - dis.shape[0]), dis]) ###补0

        predictions_dic = {'score_t': dis,
                           'feature': reps,
                           'score_tc': None,
                           'error_t': None,
                           'error_tc': None,
    ...
    def network_init(self, dim):
        net = NetModule(
            ...
            tcn_bias=self.bias,
            pretext=True if self.laa else False,
            dup=True if self.scc else False
        )
        net.to(self.device)
        return net
